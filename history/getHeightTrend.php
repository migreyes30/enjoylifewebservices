		...
		if($token == md5($PASSPHRASE)){
		    ...
		    $result = $db->command (
		            array( 
		                ...
		                "pipeline" => 
		                    array( 
		                    	array( '$project' => array( 'usuario' => 1,'historialTalla' => 1 )),
		                    	array( '$match' => array( 'usuario' => $usuario)),
		                    	array( '$unwind' => '$historialTalla'),
		                    	array( '$project' => array( 'usuario' => 1,'historialTalla.fecha' => 1,'historialTalla.talla' => 1,
		                    		'day' => array('$dayOfMonth' => '$historialTalla.fecha'),'month' => array('$month' => '$historialTalla.fecha'),
		                    		'year' => array('$year' => '$historialTalla.fecha'))),
		                        array( '$group' => array( "_id" => array("day" =>'$day',"month" =>'$month',"year" =>'$year'),
		                                    'talla' => array( '$avg' => '$historialTalla.talla')
		                                )
		                            )
		                    )
		            )
		    ...
		    foreach ($result['result'] as &$value) {
				...
				$pesoValue = $value['talla'];
				$datoFechaPeso = array("fecha" => $fecha, "talla" => $pesoValue );
